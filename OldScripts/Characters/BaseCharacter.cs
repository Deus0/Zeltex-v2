...
namespace OldCode {
	public class BaseCharacter : CharacterBasic {
		...
		public void Die() {
			...
			if (gameObject.GetComponent<AnimationUtilities.Ragdoll> ()) 	// ragdoll body has to be converted anyhow
			{
				float TimeToGetDestroyed = 30f + UnityEngine.Random.Range (0f, 60f);
				int RagdollType = 1;
				if (PewPew)
					RagdollType = 0;
				gameObject.GetComponent<AnimationUtilities.Ragdoll> ().ConvertBodyParts(RagdollType, TimeToGetDestroyed);
			}
