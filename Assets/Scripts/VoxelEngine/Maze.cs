	...
	public class Maze : WorldEditor 
	{
		...
		public void FindStartPosition(Vector3 Size) {
			for (int i = 0; i < 1000; i++) {
				...
				if (GetBlockType(CheckPosition) == PathBlockType 
				   && GetBlockType(CheckPosition+new Vector3(0,1,0)) == 0) {
		...
		public void GenerateWallsOnEdge(int MazeWallType) {
			...
			for (int k = 0; k < GetSize().z; k++) {
				if (IsOnEdge(new Vector3(i, StartLocation.y, k), GetSize()) && GetBlockType (new Vector3 (i, StartLocation.y, k)) == 0) {
					for (int z = 0; z < MazeWallHeight + 1; z++) {
								UpdateBlock (new Vector3 (i, 0 + z, k), MazeWallType);
							}
						}
					}
		}
		...
		public void FillEmptyPath(Vector3 Size, int PathBlockType, int FillBlockType) {
			...
			for (int i = 0; i < Size.x; i++)
				...
				if (GetBlockType(new Vector3(i, j, k)) == 0) {
					...
					for (int z = 0; z < MazeWallHeight + 1; z++) {
						UpdateBlock(new Vector3(i, j + z, k), FillBlockType);
					}
				}
				}
		}
		...
		void GenerateBiomeMap(Vector3 Size) {
		...
		void GeneratePaths(Vector3 Size, Vector3 StartLocation,  int PathBlockType, float Sparceness, float Linearity) {
			...
			PathDirection = 0;
			// starting block
			if (UpdateBlock(Position, PathBlockType))
				CurrentSparce++;
		...
		void  IteratePath(Vector3 Size, Vector3 StartLocation, int PathBlockType, float Sparceness, float Linearity) {
			...
			if (PathDirection <= 0 || TurnChance >= Linearity) {
				...
				if (PathDirection == 0) {
					...
					while (!HasFoundNewPosition || CheckedBlocks == MaxSparce || MaximumChecksForNewPosition >= 250) {
						...
						if (IsTypeDungeonBlock(GetBlockType(Position))) {
			...
			if (bIsPathways) 
			{
				...
				if (PathDirection == 5 || PathDirection == 6) {
					if (IsStopMaze(GetBlockType(LeftPosition)) || IsStopMaze(GetBlockType(RightPosition))) {
						PathDirection = 0;
					}
				}
				// forward, back
				...
				if (PathDirection == 1 || PathDirection == 2) {
						if (IsStopMaze(GetBlockType(BottomPosition))  || IsStopMaze(GetBlockType(BottomPosition))) {
						PathDirection = 0;
					}
				}
			}
		...
		void RemoveSinglePillars(Vector3 Size, int PathBlockType) {
			if (bIsRemoveSinglePillars)
				for (int i = 0; i < Size.x; i++)
					...
					{
						if (GetBlockType(new Vector3(i, j, k)) == 0) {
							if (GetBlockType(new Vector3(i + 1, j, k)) == PathBlockType && GetBlockType(new Vector3(i - 1, j, k)) == PathBlockType &&
							    GetBlockType(new Vector3(i, j + 1, k)) == PathBlockType && GetBlockType(new Vector3(i, j - 1, k)) == PathBlockType)
								UpdateBlock(new Vector3(i, j, k + MazeWallHeight), PathBlockType);
						}
					}
		}
