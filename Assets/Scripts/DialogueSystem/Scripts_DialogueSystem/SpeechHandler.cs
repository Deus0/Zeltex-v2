...
namespace DialogueSystem {
	public class SpeechHandler : MonoBehaviour {
		...
		void OnGUI() 
		{
			if (IsDebugMode && MyCharacter2 != null) {
		...
		void Awake () {
			MyTree.OnEndTree.AddListener(delegate{
				ExitChat();
			});
		}
		...
		public void Begin(GameObject Character2_) 
		{
			...
			//bIsTalking = true;
			IsOpen = true;
			if (MyDialogueText && MyDialogueText.gameObject.GetComponent<Text> ())
				MyDialogueText.gameObject.GetComponent<Text> ().text = "";
			else
				Debug.LogError ("No text attached to Speech Handler at " + name);
			// Dialogue Tree Stuff
			ResetDialogueTree ();
			// Sound
		...
		{
			MyCharacter2 = null;
			gameObject.GetComponent<GuiSystem.GuiManager>().EndTalk ();
		}
	...
	// GUI Stuff
		...
		public void ToggleSpeechTarget(GameObject MyCharacterTarget, bool IsOpenNew) 
		{
			if (MyCharacter2) 
			{
				...
				if (IsOpenNew)
					MyCharacterTarget.GetComponent<GuiSystem.GuiManager>().SwitchMode("Dialogue");
				else
					MyCharacterTarget.GetComponent<GuiSystem.GuiManager>().SwitchMode("");
			}
		}
		...
		{
			MyDialogueText.gameObject.GetComponent<SpeechAnimator> ().OnFinishedAnimationFunction = new UnityEvent ();
		}
		public void AddAnimationListener() 
		{
			ResetAnimationListener ();
			MyDialogueText.gameObject.GetComponent<SpeechAnimator> ().OnFinishedAnimationFunction.AddListener(OnEndAnimation);
		}
