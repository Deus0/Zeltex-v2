...
// ----- RENDERING CODES -----
	...
	public void RenderHeaderButtons() {
		...
		if (GUI.Button (new Rect (0, 0, 100, 20), "SpawnWindow")) {
			...
			MyWindow.MyDialogueLine = new DialogueSystem.DialogueData();
		}
		...
		/*if (GUI.Button (new Rect (500f, 0, 100, 20), "LoadBackground")) {
			BackgroundTexture = Resources.Load ("Background") as Texture;
		}*/
	...
	public void RenderLinks() 
	{
		...
		for (int i = 0; i < MyWindows.Count; i++) 
		{
			...
			if (MyWindows [i].MyDialogueLine.MyNext.Count == 0 && i != MyWindows.Count-1)	// render default next's
				this.DrawCurves (MyWindows [i].MyRect, MyWindows [i+1].MyRect, Color.white);
			for (int j = 0; j < MyWindows[i].MyDialogueLine.MyNext.Count; j++) 
			{
				NextIndex = MyWindows [i].MyDialogueLine.MyNext [j];
				if (NextIndex >= 0 && NextIndex < MyWindows.Count)
					this.DrawCurves (MyWindows [i].MyRect, MyWindows [NextIndex].MyRect, new Color (0.75f, 0.75f + j * 0.05f, 0.74f + j * 0.12f));
			}
		}
	}
	...
	public void GenerateWindowsFromSpeech() {
		...
		DialogueSystem.SpeechHandler MySpeech = null;
		if (MyTarget.GetComponent<CharacterSystem.CharacterSpawner> () != null)
			MyTarget = MyTarget.transform.GetChild (0).gameObject;
		//else if (MyTarget.GetComponent<CharacterSystem.Character>() != null)

		MySpeech = MyTarget.GetComponent<DialogueSystem.SpeechHandler> ();
		if (MySpeech != null) 
		{
			MyWindows.Clear ();
			int MaxXPos = MaxCountX;
			int MaxYPos = MySpeech.GetSize() / MaxCountX;
			Vector2 MidPoint = new Vector2(MyScrollArea.width/2f, MyScrollArea.height/2f);
			for (int i = 0; i < MySpeech.GetSize(); i++) 
			{
				int XPos = i % MaxCountX;
				int YPos = i / MaxCountX;

				DialogueDataGui MyWindow = SpawnNewWindow (i.ToString(), MidPoint
				                                           + new Vector2((-MaxXPos*NewWindowSize.x*MarginX)/2f, (-MaxYPos*NewWindowSize.y*MarginY)) // max size of the grid
				                                         	+ new Vector2(	NewWindowSize.x+XPos*(NewWindowSize.x*MarginX), 
				            											NewWindowSize.y+YPos*(NewWindowSize.x*MarginY)), 
				                                           NewWindowSize);
				MyWindow.MyDialogueLine = MySpeech.MyTree.GetDialogue(i);
				MyWindow.MyLinkedGameObject = MyTarget;
				MyWindow.MyIndex = i;
				MyWindow.Shrink();
			}
			MyScrollPosition = MidPoint - new Vector2(position.width/2f, position.height/2f);
		}
	}
